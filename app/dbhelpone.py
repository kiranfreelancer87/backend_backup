...
async def oi_update_one(token,oi):
    ts = await time_check()
    if ts == "1":
      token = int(token)
      oi = int(oi)
...
async def telegram_alert(raw):
   try:
      ...
      newline = "\n"
      alert = "1. Time : "+str(raw['at_time'])+newline+ \
...
async def send_alert(alert_info):
    print("============================NEW ALERT================================")
    print(alert_info)
    url = 'https://lokmaarenko.com/alert1'
    myobj = alert_info
    x = requests.post(url, json = myobj)
    print(x.text)
    try:
       with open("/home/app/alerts1.txt","a") as file:
...
async def check_poi_diff(oi,poi,ooi,opoi):
  try:
    ...
    if mside > oside:
     ...
     print("opoopopopopopopopopopopopoop: ",str(e))
...
async def condition_check(f_list,asset,expiry,lp,time_frame,lcp):
	...
	for f in f_list:
          ...
          number = f['number']
          if f['CE']['highest_value'] != -1  and f['CE']['other_value'] != -1 and f['CE']['highest_value'] > float(f['CE']['poi']) and ce_lower_strike_flag == 1 and ce_poi_diff == 1:
           print(f['PE'])
           print(f['CE'])
          ...
          if f['PE']['highest_value'] != -1 and f['PE']['other_value'] != -1 and  f['PE']['highest_value'] >  float(f['PE']['poi']) and pe_lower_strike_flag == 1 and pe_poi_diff == 1:
